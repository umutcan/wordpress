<div id="calp-event-list">
    ...
    <div class="calp-bgl">
        <div class="calp-bgr">
        <div class="calp-bg">
        <div class="calp-list-header"><?php echo _e($title, CALP_PLUGIN_NAME);?></div>
...
<script type="text/javascript">
    ...
    jQuery('#calp-navigator-prev').bind('click', function() {
        navigation_prev('<?php echo $pagination_links['middle']['2']['href'];?>&calp_post_ids=<?php echo $post_ids;?>');
    });
    jQuery('#calp-navigator-next').unbind();
    jQuery('#calp-navigator-next').bind('click', function() {
        navigation_next('<?php echo $pagination_links['middle']['4']['href'];?>&calp_post_ids=<?php echo $post_ids;?>');
    });
    var paginator = '<div class="calp-footer-bg calp-left-nav">';
        paginator += '<span id="calp-quick-prev"><a id="<?php echo $pagination_links['previous']['0']['id'] ?>" class="<?php echo $pagination_links['previous']['0']['class'] ?>" href="<?php echo esc_attr( $pagination_links['previous']['0']['href'] ) ?>&calp_post_ids=<?php echo $post_ids ?>">';
    ...
    <?php foreach ($pagination_links['middle'] as $link ) : ?>
        paginator += '<span><a id="<?php echo $link['id'] ?>" class=" <?php echo $link['class'] ?>" href="<?php echo esc_attr( $link['href'] ) ?>&calp_post_ids=<?php echo $post_ids ?>">';
    ...
    paginator += '<span class="calp-footer-dates-separator"></span>';
        paginator += '<span id="calp-quick-next"><a class=" <?php echo $pagination_links['next']['0']['class'] ?>" href="<?php echo esc_attr( $pagination_links['next']['0']['href'] ) ?>&calp_post_ids=<?php echo $post_ids ?>">';
        paginator +=  '<?php echo esc_html( $pagination_links['next']['0']['text'] ) ?>';
        paginator += '</a></span>';
    paginator += '</div>';
    jQuery('#calp-paginator-place').html(paginator);
    jQuery('#calp-today').attr('href', '#action=calp_agenda&calp_post_ids=<?php echo $post_ids ?>');
